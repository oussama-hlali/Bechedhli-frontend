...
import { CATEGORIES, getStockStatus, formatTND, CAT_COLORS } from '../data';
...
const recentActivities = [
  ...
  { icon: 'fa-file-invoice', color: '#3B82F6', text: 'Facture #2024-0847 validée — 2.4MTND', time: 'Hier' },
  { icon: 'fa-wrench', color: '#F59E0B', text: 'Maintenance onduleur client — Sousse', time: 'Hier' },
];

export default function DashboardView({ employees, stock }) {
  ...
  const stats = useMemo(() => {
    ...
    return { activeEmp, totalEmp: employees.length, totalStockValue, lowStockCount, totalSalary };
  }, [employees, stock]);
  ...
  useEffect(() => {
    ...
    g1.addColorStop(0, 'rgba(249,115,22,0.3)');
    g1.addColorStop(1, 'rgba(249,115,22,0.0)');
    const g2 = ctx1.createLinearGradient(0, 0, 0, 280);
    g2.addColorStop(0, 'rgba(59,130,246,0.2)');
    g2.addColorStop(1, 'rgba(59,130,246,0.0)');
    chart1.current = new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Revenus (M TND)',
            data: [4.2, 5.1, 6.8, 7.2, 8.5, 9.1, 10.3, 11.8, 10.5, 12.2, 13.8, 15.1],
            borderColor: '#F97316', backgroundColor: g1, fill: true, tension: 0.4,
            borderWidth: 2.5, pointRadius: 0, pointHoverRadius: 6, pointHoverBackgroundColor: '#F97316',
          },
          {
            label: 'Dépenses (M TND)',
            data: [3.1, 3.8, 4.5, 5.0, 5.8, 6.2, 7.0, 7.5, 7.2, 8.0, 8.5, 9.2],
            borderColor: '#3B82F6', backgroundColor: g2, fill: true, tension: 0.4,
            borderWidth: 2, pointRadius: 0, pointHoverRadius: 5, pointHoverBackgroundColor: '#3B82F6',
          },
        ],
      },
      options: {
        ...
        plugins: {
          legend: { display: true, position: 'top', align: 'end', labels: { color: '#64748B', font: { family: 'DM Sans', size: 12 }, usePointStyle: true, pointStyle: 'circle', padding: 16 } },
        },
        scales: {
    ...
    chart2.current = new Chart(ctx2, {
      ...
      data: {
        labels: CATEGORIES,
        datasets: [{ data: catData, backgroundColor: ['#F97316', '#3B82F6', '#10B981', '#F59E0B', '#8B5CF6'], borderWidth: 0, hoverOffset: 8 }],
      },
      options: {
        responsive: true, maintainAspectRatio: false, cutout: '72%',
        plugins: { legend: { position: 'bottom', labels: { color: '#94A3B8', font: { family: 'DM Sans', size: 11 }, usePointStyle: true, pointStyle: 'circle', padding: 12 } } },
      },
    });
  ...
  const topProducts = useMemo(() =>
    [...stock].sort((a, b) => (b.qty * b.price) - (a.qty * a.price)).slice(0, 5),
    [stock]);
  ...
  const statCards = [
    ...
    { label: 'Valeur du Stock', value: (stats.totalStockValue / 1000000).toFixed(1), suffix: 'MTND', icon: 'fa-warehouse', color: '#3B82F6', trend: '+12% vs trim. préc.', trendUp: true },
    ...
    { label: 'Masse Salariale', value: (stats.totalSalary / 1000000).toFixed(1), suffix: 'MTND/mois', icon: 'fa-money-bill-wave', color: '#10B981', trend: 'Budget stable', trendUp: true },
  ];
        ...
        {statCards.map((s, i) => (
          <div key={i} className="glass-card animate-slide-up" style={{ padding: '22px 24px', position: 'relative', overflow: 'hidden', animationDelay: `${i * 0.1}s` }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div>
                ...
                <p style={{ fontSize: 28, fontWeight: 700, lineHeight: 1 }}>
                  {s.value}<span style={{ fontSize: 14, fontWeight: 500, color: 'var(--fg-muted)', marginLeft: 4 }}>{s.suffix}</span>
                </p>
              </div>
            ...
            <p style={{ fontSize: 12, color: s.trendUp ? '#10B981' : '#F59E0B', display: 'flex', alignItems: 'center', gap: 4 }}>
              <i className={`fa-solid ${s.trendUp ? 'fa-arrow-trend-up' : 'fa-circle-info'}`} style={{ fontSize: 10 }} />
              {s.trend}
            </p>
          </div>
        ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {topProducts.map((p, i) => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#F97316', whiteSpace: 'nowrap' }}>{formatTND(p.qty * p.price)}</span>
              </div>
            ))}
          </div>
        </div>

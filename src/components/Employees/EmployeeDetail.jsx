...
export default function EmployeeDetail({ isOpen, onClose, employee, onEdit, onDelete }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Fiche Employé" width={480}>
      <div>
        <div className="flex items-center gap-4 mb-6 p-4 rounded-[14px]" style={{ background: 'rgba(0,0,0,0.02)' }}>
          ...
          <div className="flex-1">
            ...
            <p className="text-sm" style={{ color: 'var(--fg-muted)' }}>{e.role}</p>
          </div>
          <Badge status={e.status} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          {fields.map(([Icon, label, val], i) => (
            <div key={i} className="p-3 rounded-[10px]" style={{ background: 'rgba(0,0,0,0.02)' }}>
              <p className="text-xs mb-1 flex items-center gap-1.5" style={{ color: 'var(--fg-muted)' }}><Icon size={10} />{label}</p>
              <p className="font-semibold text-sm">{val}</p>
            </div>
          ))}
        </div>
        <div className="flex gap-3 justify-end mt-5">
          <button onClick={onEdit} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium cursor-pointer transition-all" style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid var(--border)', color: 'var(--fg)' }}><Pencil size={13} />Modifier</button>

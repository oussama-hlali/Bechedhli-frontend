    ...
    <div className="animate-fade-in">
      ...
      <div className="glass-card rounded-2xl p-4 mb-5 flex gap-3 flex-wrap items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input className="input-field w-full pl-10" placeholder="Rechercher par nom ou rôle..." value={search} onChange={e => setSearch(e.target.value)} style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', color: 'var(--fg)', fontSize: 14 }} />
        </div>
        <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)} className="input-field" style={{ width: 'auto', minWidth: 170, background: 'rgba(0,0,0,0.02)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', color: 'var(--fg)', fontSize: 14, cursor: 'pointer' }}>
          <option value="all">Tous les départements</option>
          {DEPARTMENTS.map(d => <option key={d} value={d} style={{ background: '#FFFFFF' }}>{d}</option>)}
        </select>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="input-field" style={{ width: 'auto', minWidth: 140, background: 'rgba(0,0,0,0.02)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', color: 'var(--fg)', fontSize: 14, cursor: 'pointer' }}>
                  ...
                  <td className="px-4 py-3.5 border-b" style={{ borderColor: 'var(--border)' }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs" style={{ color: 'var(--fg-muted)' }}>{emp.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-[13px] border-b" style={{ color: 'var(--fg-muted)', borderColor: 'var(--border)' }}>{emp.role}</td>
                  <td className="px-4 py-3.5 text-[13px] border-b" style={{ color: 'var(--fg-muted)', borderColor: 'var(--border)' }}>{emp.dept}</td>
                  <td className="px-4 py-3.5 border-b" style={{ borderColor: 'var(--border)' }}><Badge status={emp.status} /></td>
                  <td className="px-4 py-3.5 text-[13px] font-display font-semibold border-b" style={{ borderColor: 'var(--border)' }}>{formatDZD(emp.salary)}</td>
                  <td className="px-4 py-3.5 border-b" style={{ borderColor: 'var(--border)' }}>

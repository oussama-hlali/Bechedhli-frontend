      ...
      <div className="flex flex-col gap-3.5">
        {top.map((p, i) => (
          <div key={p.id} className="flex items-center gap-3">
            <span className="font-display flex items-center justify-center rounded-md text-xs font-bold" style={{ width: 22, height: 22, background: i === 0 ? 'rgba(249,115,22,0.15)' : 'rgba(0,0,0,0.04)', color: i === 0 ? '#F97316' : 'var(--fg-muted)' }}>{i + 1}</span>
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-medium truncate">{p.name}</p>
              <p className="text-xs" style={{ color: 'var(--fg-muted)' }}>{p.qty} unités</p>
            </div>

...
export function buildFactureHTML(fac, client, logoUrl) {
  const c = calcFacture(fac);
  const fmt = (n) => new Intl.NumberFormat('fr-FR').format(n) + ' TND';
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Facture ${fac.id}</title>
...
export function printFacture(fac, client, logoUrl) {
  ...
  w.document.write(buildFactureHTML(fac, client, logoUrl));
  w.document.close();
}
